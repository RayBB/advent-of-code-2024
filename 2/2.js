...
function main(rawData){
    ...
    .map(line=>{
        const withoutRemoval = allArrayDiffsInRange(line) && allNumbersInArraySameDirection(line);
        if (withoutRemoval) return true;
        // I'm too lazy to modify the lower functions to tell me which thing could be skipped and check across both.
        // So lets brute force it.
        for (let i = 0 ;i < line.length; i++) {
            const itemRemoved = line.toSpliced(i, 1)
            if (allArrayDiffsInRange(itemRemoved) && allNumbersInArraySameDirection(itemRemoved))
                return true
        }
        return false
    })
